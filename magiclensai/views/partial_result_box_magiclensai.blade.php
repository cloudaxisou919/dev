    ...
    <div class="row custom-row bg-style" >
        ...
        if ($image_storage == App\Http\Controllers\AIController::STORAGE_S3) {
            if(isset($entry->aws_path) && $entry->aws_path != null ){
                $path = $entry->aws_path;
                $image = $path;
            }elseif(isset($entry->image_local_path) && $entry->image_local_path != null && file_exists( public_path() . "/" . $entry->image_local_path ) ){
            ...
            if(isset($entry->image_local_path) && $entry->image_local_path != null && file_exists( public_path() . "/" . $entry->image_local_path )){
                $path = $entry->image_local_path;
                $image = URL::to('/').'/'.$path;
                ...
                <div class="col-6 mb-2 mt-2 ">
                    <div class="image-result group">
                        <div class="relative aspect-square rounded-lg mb-2 overflow-hidden group-hover:shadow-lg transition-all">
                                <img src="{{$image}}"  class="w-full h-full aspect-square object-cover object-center img-fluid" alt="{{$entry->input}}">
                                <div class="flex items-center justify-center gap-2 w-full h-full absolute top-0 left-0 opacity-0 transition-opacity group-hover:!opacity-100">
                                    <div class="flex flex-col">
                                        <div>
                                            <a href="<?php echo $is_cdn_path == true ? $image : route('download.image')."/?url=".$image; ?>" target="_blank" class="btn items-center justify-center w-9 h-9 p-0 <?php if( $is_cdn_path == true){ ?>download<?php } ?> ">
                                            ...
                                            <a href="{{ LaravelLocalization::localizeUrl(route('dashboard.user.magiclensai.image.delete', ['id' => $entry->id])) }}" onclick="return confirm('Are you sure?')" class="btn items-center justify-center w-9 h-9 p-0">
                                        ...
                                        <div class="btn-group mt-3 mb-3 justify-center" role="group"><a href="{{route('dashboard.user.magiclensai.generator', 'ai_image_generator')}}">
                                                <button class="btn btn-light btn-default variation" data-id= "{{$entry->butonData->button_id}}" data-index = "{{$entry->image_index}}" data-variation="{{$entry->id}}" autofocus="autofocus" id= "openai_generator_button" style="background-color: #330582;color:white" >
                                                   Variation
                                                </button>
                                            </a>
                                        </div>
        ...
        @endforeach
        <p class=" mt-3  text-muted d-grid gap-2 text-center mb-4 mx-auto">{{$entry->created_at->diffForHumans()}}</p>
    </div>
 </div>